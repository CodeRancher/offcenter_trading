...
#include "PersistOandaForexDataToCSV.hpp"
...
int main(int argc, char **argv)
{
	try {
		...
		std::cout << "Persist Oanda Forex Data to CSV App!" << std::endl;

		LOG(INFO) << "--------------------------------------------------------------------------------";
		LOG(INFO) << "Persist Oanda Forex Data to CSV App!";

		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::ToFile, "true");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::ToStandardOutput, "false");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Format, "%datetime:%thread [%level] %msg");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Filename, "./PersistOandaForexDataToCSV.log");
